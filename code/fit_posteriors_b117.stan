...
parameters {
  ...
  real<lower=0.25, upper=wpmax> wp[n_id];  // Onset-to-peak time
  real<lower=2, upper=wrmax> wr[n_id];  // Peak-to-recovery time 
...
model {
  ...
  for(i in 1:n_id){
    if(b117[i] == 1){
      dp[i] ~ normal(dpmeanB, dpsd) T[0, lod];
      wp[i] ~ normal(wpmeanB, wpsd) T[0.25, wpmax];
      wr[i] ~ normal(wrmeanB, wrsd) T[2, wrmax];
      } else {
